...
def main():
    ...
    # Model
    # Load checkpoint.
    pre_trained_audio_net = resnet50(pretrained=True, conv1_channel=3)
    pre_trained_rgb_net   = resnet50(pretrained=True, conv1_channel=3)
    ...
    for epoch in range(start_epoch,end_epoch):
        print('\nEpoch: %d '%(epoch))
        print('Train')
        mix_train_two_stream(epoch, audio_net, rgb_net, optimizer, trainloader, criterion, 1, args)
        mix_train_two_stream(epoch, audio_net, rgb_net, optimizer, trainloader, criterion, 2, args)
        scheduler.step()
        print('Test')
